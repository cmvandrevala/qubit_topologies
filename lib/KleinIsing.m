function [total_magnetic_field, total_energy] = KleinIsing(temperature)
...
for i=1:number_of_potential_spin_flips,
  ...
  neighbors(cells_on_side_of_grid,cells_on_side_of_grid) = grid(cells_on_side_of_grid,cells_on_side_of_grid-1) + grid(cells_on_side_of_grid,1) + grid(cells_on_side_of_grid-1,cells_on_side_of_grid) + grid(1,1);
